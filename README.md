...
the scripts assume you are already in a tmux session named claude-vm
 ...
 - `launch <project name>`: spins up a project container in a new tmux window on the vm, or switches to the window if the project is already running. handles container naming, network setup, port forwarding, and bind mounting of common config and project-specific files. the entrypoint starts a container-scoped tmux session and launches claude automatically.
